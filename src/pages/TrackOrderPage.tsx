...
import { Search, Package, Clock, CheckCircle, Truck, FileText, Printer, Zap, Package2, MapPin } from 'lucide-react';
...
const TrackOrderPage: React.FC = () => {
  ...
  const handleTrackOrder = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Check for specific Lead ID format or default to "In Production" (pressing stage)
      if (leadId.toUpperCase() === 'MD5X9O8ORN02SB' || leadId.length >= 8) {
        // Default to "pressing" stage for valid Lead IDs
        setOrderStatus(statusOptions[3]); // Pressing stage
      } else {
        // For shorter IDs, use hash-based logic for demo
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-xs px-2 py-1 rounded-full ${
                                ...
                                {isActive ? 'Current' : isCompleted ? 'Complete' : 'Pending'}
                              </span>
                            </div>
                            
                            {isActive && (
                              <p className="text-rb-gray-400 text-sm mt-1">
                                {status.description}
                              </p>
                            )}
                          </div>
                    ...
                    <div>
                      <p className="text-rb-gray-300 text-sm">
                        <strong>Estimated Completion:</strong> {
                          ...
                          orderStatus.status === 'ready-delivery' ? '1-2 days' :
                          '3-5 days'
                        }
                      </p>
                      <p className="text-rb-gray-300 text-sm mt-1">
                        <strong>Production Facility:</strong> Johannesburg, SA
                      </p>
                    </div>
                  </div>
                </div>
